...
const fastlyApiKeyError = `[error] ${fastlyApiKeyDescription} is required.`;
const fastlyServiceIdError = `[error] ${fastlyServiceIdDescription} is required.`;
...
program
  ...
  .action(() => {
    ...
    if (!fastlyServiceIdPresent(program)) {
      return;
    }
...
function fastlyApiKeyPresent(program) {
  if (!program.apikey) {
    console.error(chalk.red(fastlyApiKeyError));
    return false;
  }
  return true;
}
...
function fastlyServiceIdPresent(program) {
  if (!program.serviceid) {
    console.error(chalk.red(fastlyServiceIdError));
    return false;
  }
  return true;
}
